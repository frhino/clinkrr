...
st.markdown("Search across all historical permit records. Adjust the sidebar timeline slider to find vintage filings.")
...
# 2. CRITICAL UPGRADE: Permit Filing Timeline Filter 
start_filing_year, end_filing_year = st.sidebar.slider(
    "Permit Filing Year Range",
    min_value=1980, 
    max_value=2026, 
    value=(2001, 2026) # Defaults to search from 2001 onwards
)

# 3. Structural Construction Type Filter
...
def fetch_sf_data(statuses, filter_method, keywords, max_built_year, start_yr, end_yr):
    if not statuses:
        return pd.DataFrame()
    ...
    # Inject filing date timeline restrictions straight into the SoQL query
    where_clause = (
...
if st.sidebar.button("Run Search / Refresh Data"):
    with st.spinner("Scanning complete DataSF catalog..."):
        results_df = fetch_sf_data(selected_statuses, construction_filter, keyword_list, target_year, start_filing_year, end_filing_year)
        ...
        if not results_df.empty:
            ...
            st.download_button(
                label="📥 Download Timeline Results as CSV",
                data=csv,
                file_name=f"sf_historic_permits_{start_filing_year}_to_{end_filing_year}.csv",
                mime='text/csv',
            )
        else:
            st.info("No records found matching your specified timeframe parameters.")
else:
